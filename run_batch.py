...
def run_experiment(
    ...
    **method_kwargs,
    ...
    if method == "random":
        ...
        generator = create_generator(
            "jl", seed=seed, oversampling_factor=method_kwargs.get("oversampling", 2.0)
        )
    elif method == "energy":
        generator = create_generator(
            "energy",
...
def main():
    parser = argparse.ArgumentParser(
        description="Run batch vector generation experiments"
    )
    ...
    # Quick test option
    parser.add_argument(
        "--quick", action="store_true", help="Run quick test with small values"
    )

    # Output options
    parser.add_argument(
        "--output-dir", type=Path, default=Path("results"), help="Output directory"
    )
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument(
        "--verbose", action="store_true", help="Show progress for each generation"
    )
    ...
    parser.add_argument(
        "--device", type=str, choices=["cuda", "cpu"], help="Device for energy method"
    )
    ...
    print(
        f"Total experiments: {len(args.dimensions) * len(args.counts) * len(args.methods)}"
    )
    print(f"{'='*60}\n")
    ...
    fig3 = plot_time_comparison(
        all_results, output_path=args.output_dir / "time_analysis.png"
    )
