...
def run_from_config(config: Dict[str, Any], output_dir: Path, verbose: bool = False) -> List[Dict[str, Any]]:
            ...
            for method_name, method_params in methods_config.items():
                ...
                try:
                    ...
                    if params.get('save_vectors', False):
                        ...
                        vectors_file = vectors_dir / f"{method_name}_d{dimension}_n{count}.npy"
...
def main():
    parser = argparse.ArgumentParser(description='Run experiments from configuration file')
    ...
    if config.get('generate_plots', True):
        ...
        plot_dimension_comparison(
            results,
            dimensions,
            output_path=output_dir / 'dimension_comparison.png'
        )
        
        plot_time_comparison(
            results,
            output_path=output_dir / 'time_analysis.png'
        )
