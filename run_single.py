...
def main():
    parser = argparse.ArgumentParser(
        description="Generate almost orthogonal vectors using a single method"
    )
    ...
    parser.add_argument(
        "--count", type=int, required=True, help="Number of vectors to generate"
    )
    ...
    parser.add_argument(
        "--save-vectors", action="store_true", help="Save generated vectors to file"
    )
    parser.add_argument(
    ...
    if args.method == "random":
        generator = create_generator(
            "random", seed=args.seed, oversampling_factor=args.oversampling
        )
    elif args.method == "jl":
        generator = create_generator(
            "jl", seed=args.seed, oversampling_factor=args.oversampling
        )
    ...
    results_file = (
        args.output_dir
        / f"{args.method}_d{args.dimension}_n{args.count}_seed{args.seed}.json"
    )
    ...
    if args.save_vectors:
        vectors_file = (
            args.output_dir
            / f"{args.method}_d{args.dimension}_n{args.count}_seed{args.seed}_vectors.npy"
        )
