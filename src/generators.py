...
class RandomSamplingGenerator(BaseGenerator):
    ...
    def _prune_vectors(self, vectors: np.ndarray, target_count: int, verbose: bool) -> np.ndarray:
        """
        ...
        for step in pbar:
            ...
            with torch.no_grad():
                ...
                if verbose and isinstance(pbar, tqdm):
                    pbar.set_postfix({'ε': f'{current_epsilon:.4f}', 
                                     'best_ε': f'{best_epsilon:.4f}'})
...
def create_generator(method: str, **kwargs) -> BaseGenerator:
    ...
    if method == 'random':
        ...
        raise ValueError(f"Unknown method: {method}. Choose from 'random', 'jl', 'energy'")
